...
```bash
kubectl port-forward svc/rappmanager 8080:8080 -n ridenext-nonrt
```
...
```bash
# Check pods
kubectl get pods -n ridenext-nonrt | grep rappmanager

# Check service
kubectl get svc -n ridenext-nonrt | grep rappmanager

# Check logs
kubectl logs -f deployment/rappmanager -n ridenext-nonrt
...
Check pod events:
```bash
kubectl describe pod -l app.kubernetes.io/name=rappmanager -n ridenext-nonrt
```

Check logs:
```bash
kubectl logs -l app.kubernetes.io/name=rappmanager -n ridenext-nonrt
```
...
Check PVC status:
```bash
kubectl get pvc -n ridenext-nonrt | grep rappmanager
```
...
```bash
kubectl get pods -n ridenext-nonrt | grep -E "informationservice|capifcore|policy"
```
...
helm uninstall rappmanager -n ridenext-nonrt

# Clean up PVC
kubectl delete pvc rappmanager-pvc -n ridenext-nonrt
```
