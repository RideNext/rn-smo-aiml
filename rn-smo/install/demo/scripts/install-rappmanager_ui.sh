...
HELM_DIR="${DEMO_DIR}/helm/rappmanager_ui"
...
helm upgrade --install rappmanager_ui ${HELM_DIR} \
    --namespace ${NAMESPACE} \
    --wait \
    --timeout 5m
...
if [ $? -eq 0 ]; then
    ...
    kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=rappmanager_ui -n ${NAMESPACE} --timeout=300s
    
    if [ $? -eq 0 ]; then
        echo "✓ rApp Manager is ready"
        echo ""
        echo "Service details:"
        kubectl get svc -n ${NAMESPACE} | grep rappmanager_ui
        echo ""
        echo "Pod status:"
        kubectl get pods -n ${NAMESPACE} | grep rappmanager_ui
    else
        echo "⚠ rApp Manager pod is not ready yet. Check status with:"
        echo "  kubectl get pods -n ${NAMESPACE} | grep rappmanager"
    fi
else
    echo "✗ Failed to install rApp Manager UI"
    exit 1
fi
