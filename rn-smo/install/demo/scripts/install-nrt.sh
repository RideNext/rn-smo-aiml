...
. scripts/helper-scripts/kube_get_nodeport.sh
. scripts/helper-scripts/wait_for_server_ok.sh
. scripts/helper-scripts/get_influxdb2_token.sh
...
##################################################################################
helm install --wait --create-namespace -n ridenext-nonrt  nrt-base-0 helm/nrt-base-0
...
helm install --wait strimzi-kafka-crds -n ridenext-nonrt  strimzi/strimzi-kafka-operator --version $STRIMZIKAFKAOPERATORVERSION

cp opa-rules/bundle.tar.gz helm/nrt-base-1/charts/opa-rule-db/data

helm install -n ridenext-nonrt nrt-base-1 helm/nrt-base-1

echo "Waiting for influx db - there may be error messages while trying..."
retcode=1
while [ $retcode -eq 1 ]; do
    retcode=0
    CONFIG=$(kubectl exec -n ridenext-nonrt influxdb2-0 -- influx config ls --json)
    if [ $? -ne 0 ]; then
        retcode=1
        sleep 1
    elif [ "$CONFIG" == "{}" ]; then
        echo "Configuring db"
        kubectl exec -n ridenext-nonrt influxdb2-0 -- influx setup -u admin -p mySuP3rS3cr3tT0keN -o est -b pm-bucket -f
        if [ $? -ne 0 ]; then
            retcode=1
            sleep 1
        fi
...
INFLUXDB2_TOKEN=$(get_influxdb2_token influxdb2-0 ridenext-nonrt | base64 $B64FLAG)
PATCHDATA='[{"op": "add", "path": "/data/token", "value": "'$INFLUXDB2_TOKEN'"}]'
kubectl patch secret influxdb-api-token -n ridenext-nonrt --type json -p "$PATCHDATA"

echo "Wait for kafka"
_ts=$SECONDS
until $(kubectl exec -n ridenext-nonrt kafka-client -- kafka-topics --list --bootstrap-server kafka-1-kafka-bootstrap.ridenext-nonrt:9092 1> /dev/null 2> /dev/null); do
...
for __topic in $__topics_list; do
    create_topic kafka-1-kafka-bootstrap.ridenext-nonrt:9092 $__topic 10
done
...
echo "Wait for SDNR"
#_ts=$SECONDS
#until [ "$(curl -s -o /dev/null -w "%{http_code}" http://localhost:31769/)" = "401" ]; do
#    echo -ne "  $(($SECONDS-$_ts)) sec, retrying at $(($SECONDS-$_ts+5)) sec                        $SAMELINE"
#    sleep 5
#done
echo ""
...
helm install --wait -n ridenext-nonrt  nonrtricgateway helm/nonrtricgateway
echo ""
...
helm install --wait -n ridenext-nonrt controlpanel helm/controlpanel
echo ""
...
# Get the InfluxDB token for use in the kafka-producer-pm-json2influx component
. scripts/helper-scripts/get_influxdb2_token.sh
export INFLUXDB2_TOKEN=$(get_influxdb2_token influxdb2-0 ridenext-nonrt)
check_error $? "Failed to get InfluxDB token"

# Create pm_data bucket if it doesn't exist
. scripts/helper-scripts/create_influxdb2_bucket.sh
create_influxdb2_bucket influxdb2-0 ridenext-nonrt pm_data
check_error $? "Failed to create pm_data bucket in InfluxDB"
...
helm install --wait -f helm/global-values.yaml -n ridenext-nonrt nrt-pm helm/nrt-pm
...
echo "Do: 'kubectl get po -n ridenext-nonrt' and verify that all pods are in status Running"
