...
- Kubernetes cluster with InfluxDB running in `nonrtric` namespace
- Helm 3.x installed
...
```bash
INFLUXDB_TOKEN=$(kubectl exec -n nonrtric influxdb2-0 -- influx auth list --org est --json | jq -r '.[0].token')
```
...
1. **InfluxDB Connection Failed**
   - Verify InfluxDB is running: `kubectl get pods -n nonrtric | grep influxdb`
   - Check token: `kubectl logs -n nonrtric grafana-xxx`

2. **Dashboard Not Loading**
   - Check ConfigMap: `kubectl get configmap -n nonrtric grafana-dashboards`
   - Restart Grafana: `kubectl delete pod -n nonrtric -l app=grafana`

3. **Storage Issues**
   - Check PVC: `kubectl get pvc -n nonrtric grafana-pvc`
...
```bash
# Check Grafana logs
kubectl logs -n nonrtric -l app=grafana

# Access Grafana pod
kubectl exec -it -n nonrtric -l app=grafana -- bash

# Port forward for local access
kubectl port-forward -n nonrtric svc/grafana 3000:3000
```

## Uninstallation

```bash
helm uninstall grafana -n nonrtric
kubectl delete pvc grafana-pvc -n nonrtric
```
...
1. **InfluxDB** (Default)
   - Bucket: `pm_data`
   - Organization: `est`
   - URL: `http://influxdb2.nonrtric:8086`
